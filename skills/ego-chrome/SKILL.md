...
Keep predictable observations, actions, waits, extraction, and verification in one invocation. Start a second invocation only when the prior output is needed to choose an unpredictable next action or when the user must intervene.
...
```powershell
@'
const task = await taskSpaces.useOrCreate('update example profile')
...
In this MVP, task spaces are logical labels inside one CLI invocation. They do not isolate cookies, local storage, or site sessions.
...
- `ELEMENT_NOT_FOUND`: verify load state, then take one targeted snapshot or use a stable selector.
